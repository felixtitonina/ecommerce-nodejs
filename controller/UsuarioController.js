...
class UsuarioController {
    ...
    showCompleteRecovery(req, res, next) {
        if (!res.query.token) return res.render("recovery", { error: "Token não identificado", success: null })
    ...
    completeRecovery(req, res, next) {
        ...
        Usuario.findOne({ "recovery.token": token }).then(usuario => {
            if (!usuario) return res.render("recovery", { error: "Usuario não identificado", sucess: null })
